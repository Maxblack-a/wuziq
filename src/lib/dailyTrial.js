...
export async function getDailyTrialStatus() {
  const { data, error } = await supabase.rpc("get_daily_trial_status");
...
// 返回当前的评分/林墨评分——前端拿这两个分数去初始化这一局的强度旋钮。
// 体力不够时 RPC 会直接抛错('体力不足'),调用方 catch 住给提示即可。
export async function startDailyTrial() {
  const { data, error } = await supabase.rpc("start_daily_trial");
  if (error) throw error;
  const row = Array.isArray(data) ? data[0] : data;
  return {
    stamina: row.out_stamina,
...
export async function finishDailyTrial(result, quality) {
